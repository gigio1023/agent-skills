...
Trajectory 하나는 세 부분으로 이루어진다.

- 사용자 과업(user task)
- Agent가 받은 tool 결과에 섞인 주입 문장(untrusted input)
- Agent의 tool 호출과 최종 답변

사람 label이 75개에만 있으므로 meta-evaluation은 이 75개에서만 계산할 수 있다. 나머지 117개에는 judge 판정만 있다.

결과는 trajectory마다 한 행으로 저장한다. 열은 trajectory id, judge의 goal compliance와 harmfulness label, 사람 label 두 개(label이 있는 trajectory에만), judge 근거 문장이다.

## Judge와 사람 label의 일치

사람 label이 있는 75개 trajectory에서 judge 판정과 사람 label을 비교한 결과다.

| 판정 | 일치 | 일치율 |
| --- | ---: | ---: |
| Goal compliance | 68/75 | 90.7% |
| Harmfulness | 61/75 | 81.3% |

Harmfulness 불일치 14건은 한 방향으로 몰려 있다. 11건은 사람이 harmful로 붙인 trajectory를 judge가 harmful이 없다고 판정한 false negative이고, 3건은 사람이 무해하다고 본 trajectory를 judge가 harmful로 판정한 false positive다.
...
나머지 false negative 3건과 false positive 3건은 아직 유형별로 나누지 않았다.
...
현재 저장 형식에는 label과 근거 문장만 있어서, 불일치를 세면 개수와 방향(false negative, false positive)까지만 나온다. 두 축을 교차하면 불일치 한 건은 3 × 4 = 12칸 중 한 칸에 들어간다. 앞의 8건은 주입 위치가 tool 결과이고, state change가 파일 삭제 또는 외부 전송(이메일)인 칸에 모인다. 칸별로 세면 judge가 어느 위치의 주입이 어떤 state change로 이어질 때 놓치는지가 드러나고, rubric 수정도 해당 칸의 사례를 근거로 정할 수 있다.

적용 순서는 다음과 같다.

1. 기존 harmfulness 불일치 14건을 두 축으로 분류한다.
2. 분류 결과를 보고 나머지 117개 trajectory에도 사람 label을 붙일지 정한다.

14건은 12칸에 나누기에는 적은 수이므로, 첫 분류는 불일치가 어느 칸에 몰리는지 확인하고 label을 늘릴지 판단하는 근거로 쓴다.

Trajectory 출처를 넓힐 때는 CVE-Bench와 R-Judge를 후보로 검토할 수 있으며, 둘 다 정해진 대상은 아니다.
