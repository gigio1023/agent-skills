...
description: >
  ...
  models at full effort, and state each lane's resolved settings before
  spawning. Also use to bring Astra in as a judgment lane under a Sol or Terra
  lead. NOT for deciding whether to delegate (orchestrate-subagents), for
  missions launched through codex-delegate, or for a Claude Fable lead
  (fable5-model-routing).
---

# GPT-6 Astra Model Routing

Assign a model and a reasoning effort to every lane a GPT-6 Astra lead opens, and keep Astra itself for judgment. The scope is the lead model, not the harness: the policy applies wherever an Astra lead can spawn subagents, and the harness only changes how a choice is expressed. `references/harness-adapters.md` holds those mechanics, with Codex as the worked example verified against its source, Hermes and OpenCode as further examples, and a procedure for any other harness.
...
- **Astra leads.** The policy stands for the whole session. Each time the lead decides to delegate, the lane gets a model and an effort chosen for its tier, stated before the spawn. The policy applies one level down only: a subagent does not re-apply it to its own children.
- **A Sol or Terra lead with Astra as a judgment lane.** Apply on request, when the user names this skill or asks to put Astra on the judgment, or when a mission packet grants it. Astra is then a critic and strategist, never an executor: it receives the evidence and the question, returns a verdict with the top risks and specific fixes, and the lead applies or explicitly rebuts each note. Give the lane a countable cap. This shape keeps Astra off the session and is the structural answer to its cost.
- **A harness that cannot vary subagent model or effort.** The skill still applies. Decide whether the inherited lane is acceptable for the tier, and say so.
...
## Default Lanes for an Astra Lead

The lane names are the role definitions in `assets/codex/agents/`; the adapter reference says how to install them in Codex and how to express the same lanes elsewhere. The model split follows OpenAI's own subagent guidance for the GPT-5.6 family: the flagship for ambiguous multi-step work, Terra for exploration and read-heavy scans, Luna for clear, repeatable, high-volume work. The efforts follow the floor.

| Tier | Default lane | Notes |
|------|--------------|-------|
| Mechanical collection | `luna-clerk`: GPT-5.6 Luna at `xhigh` | Cheap enough that full effort costs little |
| Bounded execution, read-heavy exploration or scans | `terra-scout`: GPT-5.6 Terra at `xhigh` | Also the session default worker when the lead names no lane |
...
The tier table is only executable where the harness lets a lane carry its own model and effort, or lets the session name a default worker. `assets/codex/` ships a config snippet with the `[agents]` defaults and four role files for Codex; `references/harness-adapters.md` says where they go, what to check first, and how to express the same lanes in another harness. Installation changes user configuration, so propose it and wait for approval; do not edit configuration unasked. Read the harness's model catalog at install time and record the defaults and effort ladders you found, with the date, in `references/source-notes.md`; they change without notice.
...
| `assets/codex/` | When installing lanes in Codex | The `[agents]` defaults snippet and four role files, each carrying a model and an effort |
