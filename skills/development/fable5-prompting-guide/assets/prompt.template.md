...
Produce <user-visible outcome>.

## Definition of Done

- <Observable result>
- <Required evidence, artifact, or state change>
- <Quality, scope, or safety invariant>
...
- For analysis-only requests, inspect and report; do not implement.
- For change requests, perform <reversible in-scope work> and validate it.
- Do not add <features, refactors, abstractions, or adjacent work> beyond scope.
- Pause only for <destructive action, real scope change, or user-only input>.

# Tools, Delegation, and State
...
- Validate with <test, render, review, or external state check>.
- If <required evidence> is missing, use <bounded fallback> or report the blocker.
- Stop when <definition of done> is verified.
- Do not end on a promise to run a tool or perform work that can be done now.
