...
- Comments are for useful discussion, decisions, blockers, and handoffs. A body rewrite does not need an announcement or a second copy of the removed history.
- The rules under "State The Claim, Justify Every Link". They follow the reader-access principles now in `technical-report-writing`'s sharing-and-delivery reference (formerly Gigio Pack's `share-internal-doc`) and answer an observed failure: issues that pointed at paths on the author's machine or a server, and at piles of loosely related links, instead of stating their findings.
