...
description: >
  ...
  craft; share-internal-doc adds recipients and delivery. NOT for PR copy (draft-pr), issues
  (write-issue), diagrams (technical-diagram), or dashboards (insight-dashboard).
---
...
Write for a reader who lacks the drafting conversation, and select and organize information before polishing sentences. Infer reader, task, sources, language, and medium from context, and ask only about gaps that change meaning or authority. A new document returns the artifact, a review returns findings and edits nothing, and a revision changes only what was authorized. In a user-edited copy, the current copy is the baseline; compare it with the closest earlier version to see which functions the user removed and kept. Only the user's edits show preference, so isolate changes other agents made. Preserve established terms, templates, and voice, and change the system itself only when authorized. The Korean clarity floor in [Korean writing](references/korean-writing.md) also covers ordinary Korean answers and progress reports.
