...
3. **사용자 본인 워크스페이스 글**: 작성자 본인 vocabulary (~/git/<workspace>/ 안 README / AGENTS.md / CLAUDE.md / docs/*.md, 예: /Users/user/git/ad-agent-metrics/)
...
### Step 6: 사용자 본인 워크스페이스 검색 (가장 가치 높음)
...
평가:
- 본인 글에 등장 + 그 의미로 쓰임 → **+2 점** (가장 가중치 큼. 작성자 본인 vocabulary 라 자연)
- 본인 글에 등장하지만 다른 의미 → 0 점
- 본인 글에 0 건 → LLM 이 새로 박은 표현, 의심 강화 → -1 점

본인 워크스페이스 안에 비슷한 도메인 글이 여러 개 있으면 빈도까지 측정. 1 회만 등장하고 LLM 영향 가능성 있으면 가중치 절반.
